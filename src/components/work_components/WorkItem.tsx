...
const WorkItem = ({year, duration, company, title, details, skills}: Props) => {
    ...
    return (
        <ol className={'flex flex-col md:flex-row relative border-l border-stone-200 rounded-md'}>
            <li className={'mb-10 ml-4'}>
                ...
                <p className={'flex flex-wrap gap-4 flex-row items-center justify-start'}>
                    ...
                    <div className={'flex flex-col'}><span
                        className={'text-lg font-semibold text-[#efedff]'}>{company}</span>
                        <div className={'flex flex-row space-x-4 justify-between'}>
                            <span
                                className={'text-base font-semibold text-[#efedff]'}>{title}</span>
